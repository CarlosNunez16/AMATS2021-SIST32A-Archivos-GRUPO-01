<?php
    if(isset($_POST["Siguiente"]))
    {
        ...
        $tabla = "usuarios";
        $consulta = $objeto -> SQL_consulta_condicional($tabla, "*", "contraseña like '".$datos."' and carnet like '".$_SESSION["Estudiante_Empleado"][0]."'");
                    ...
                    <div class='col-4 m-3 s-1 p-3 border border-dark rounded-3 d-block' style='background-color:#F5F5F5'>
                        <form class='row g-3 needs-validation' method='post'>
                            ...
                            <div class='col-12'>
                                ...
                                <a class='btn btn-danger' href='Empleado_Estudiante.php?pagina=Prestamo.php&opcion=all'>Cancelar</a>
                            </div>
                        </form> 
                    </div>
                </div>
...
if(isset($_POST["Guardar"]))
{
    ...
    {
        $condicion = "carnet = ".$_SESSION["Estudiante_Empleado"][0]."";
		$rs = $objeto -> SQL_modificar("usuarios", $claveNew, $condicion);
		echo "<script>alert('CONTRASEÑA MODIFICADA'); window.location='?pagina=Prestamo.php&opcion=all';</script>";
    }
    else
    {
